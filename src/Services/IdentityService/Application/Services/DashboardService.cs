...
namespace IdentityService.Application.Services
{
    public class DashboardService : IDashboardService
    {
        ...
        public async Task<ApiResponse> GetDashboardDataAsync()
        {
            try
            {
                ...
                var newStudents = await _context.Users
                    .Where(u => u.CreatedAt.Year == currentYear && !_context.UserRoles.Any(ur => ur.UserId == u.Id && ur.RoleId == adminRoleId))
        ...
        public async Task<ApiResponse> GetDashboardNotificationsAsync()
        {
            try
            {
                ...
                var notifications = pendingRequests.Select(r => new
                {
                    id = r.Id,
                    type = "CourseRequest",
                    sender = r.Instructor?.FullName ?? "Instructor",
                    courseName = r.Course?.Name ?? "Unknown Course",
                    createdAt = r.CreatedAt,
                    isRead = false
                }).ToList();

                return new ApiResponse("Success", _localizer["Success"].Value, new { notifications }, true);
            }
            catch (Exception ex)
            {
                return new ApiResponse("Error", ex.Message, null, false);
